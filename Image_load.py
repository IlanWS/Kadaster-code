...
def load_images():
    ...
    for i in range(number_of_data_pairs):
        ...
        input_image_array[i] = np.delete(np.delete(np.delete(image_array1, np.s_[input_image_height::], 0),np.s_[input_image_width::], 1),np.s_[3::], 2)

        #image_path2 = "".join([output_data_dir, "/image_", str(i),".jpg"])
        image_path2 = "".join([output_folder, "/image_", str(i), ".jpg"])
        image2 = Image.open(image_path2)
        image_array2 = np.array(image2)
        target_image_array[i] = np.delete(np.delete(np.delete(image_array2, np.s_[input_image_height::], 0),np.s_[input_image_width::], 1),np.s_[3::], 2)
